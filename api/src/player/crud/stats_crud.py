    ...
    if len(player_stats) == 0:
        ...
        if player_data:
            ...
            stats_create = StatsCreate(
                season=-1, team_id=-1, stats=career_stats["stats"]
            )
        ...
        for season_stats in year_by_year_stats["stats"]:
            stats_create = StatsCreate(
                season=season_stats["season"], team_id=-1, stats=season_stats["stats"]
            )
            create_stats(db, stats_create, mlb_id)
...
def create_stats(db: Session, stats: StatsCreate, mlb_id: int):
    db_stats = Stats(
        mlb_id=mlb_id, season=stats.season, team_id=stats.team_id, stats=stats.stats
    )
