...
<template>
        <v-container>
            <v-row>
                <v-col>
                    <div class="text-h5 text-center mt-2 bg-primary">{{ details.year }}</div>
                    
                    <v-table>
                        <thead>
                            <tr>
                                <th class="text-center" v-for="pitch in details.types" :key="pitch.code">
                                    <v-tooltip :text="findTooltip(pitch.code)" location="top">
                                        <template v-slot:activator="{ props }">
                                            <v-btn variant="plain" v-bind="props">{{ pitch.code }}</v-btn>
                                        </template>
                                    </v-tooltip>
                                </th>
                            </tr>
